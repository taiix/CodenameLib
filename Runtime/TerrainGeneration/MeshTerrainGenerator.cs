...
namespace CodenameLib.ProceduralTerrain
{
    public static class MeshTerrainGenerator
    {
        ...
        public static MeshTerrainResult GenerateMeshTerrain(TerrainSettings settings)
        {
            try
            {
                Debug.Log($"[TerrainGen] ▶️ Generating terrain. Seed={settings.seed}, Res={settings.resolution}");

                float[,] heightmap = GenerateHeightmapWithComputeShader(settings);
                ValidateHeights("After heightmap generation", heightmap);

                Mesh mesh = CreateMeshFromHeightmap(heightmap, settings);
                Texture2D heightmapTexture = CreateHeightmapTexture(heightmap);

                Debug.Log("[TerrainGen] ✅ Mesh creation complete.");
                return MeshTerrainResult.SuccessResult(mesh, heightmapTexture, heightmap);
            }
        ...
        private static float[,] GenerateHeightmapWithComputeShader(TerrainSettings settings)
        {
            ...
            try
            {
                ...
                GeneratePerlinNoiseGPU(computeShader, settings, perlinData, resolution);
        ...
        private static void GeneratePerlinNoiseGPU(
            ...
            ComputeBuffer perlinData, int resolution)
        {
            int kernelIndex = computeShader.FindKernel("CSMain");

            computeShader.SetBuffer(kernelIndex, "perlinData", perlinData);
            computeShader.SetInt("width", resolution);
